...
require_once __DIR__ . '/../shared/grading_engine.php';
...
$attPct = $myGrade['components']['attendance'] ?? ($totalSessions > 0 ? round(($totalAttPoints / ($totalSessions * 2)) * 100, 1) : null);
              ...
              <table class="custom-table">
                ...
                <tbody>
                  ...
                  $compKeys  = [
                      ...
                      'exam'        => 'exam_pct',
                      'written'     => 'written_pct',
                      'deportment'  => 'deportment_pct',
                      'attendance'  => 'attendance_pct'
                  ];
                  foreach($compNames as $comp => $label):
                    $raw = $myGrade['raw'][$comp] ?? null;
                    $total = $myGrade['total_items'][$comp] ?? 0;
                    $trans = $myGrade['transmuted'][$comp] ?? null;
                    $weight = $weights[$compKeys[$comp]] ?? 0;
                    $contrib = $myGrade['weighted'][$comp] ?? null;
                  ?>
                  <tr>
                    <td><strong><?php echo $label; ?></strong></td>
                    <td><?php echo $weight; ?>%</td>
                    <td><?php echo ($raw !== null && $total > 0) ? ($raw . ' / ' . $total) : '—'; ?></td>
                    <td style="font-weight:700; color:<?php echo $trans>=75?'#166534':'#0f172a'; ?>;"><?php echo $trans !== null ? number_format($trans, 2) : '—'; ?></td>
                    <td style="font-weight:700; color:#1792bb;"><?php echo $contrib !== null ? number_format($contrib, 2).'%' : '—'; ?></td>
                  </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
      ...
      <div class="record-card">
        ...
        <div class="table-responsive">
          <table class="custom-table">
            ...
            <tbody>
              <?php 
              $myDepCols = array_filter($columns, fn($c) => ($c['component'] === 'deportment'));
              if(empty($myDepCols)): ?>
              ...
              <tr>
                ...
                <td style="text-align:center; font-weight:700;">
                  <?php echo $sc !== null ? ($sc . ' / ' . $colMax) : '—'; ?>
                </td>
                <td style="text-align:center;">
                  <?php if($sc !== null): 
                    $pct = ($colMax > 0) ? ($sc / $colMax) * 100 : 0;
                  ?>
                    <span class="status-badge <?php echo $pct >= 75 ? 'badge-pass' : 'badge-fail'; ?>">
          ...
          <table class="custom-table">
            ...
            <tbody>
              ...
              <?php foreach($sessions as $sess): 
                $statusHtml = '<span class="status-badge badge-fail"><i class="fa fa-times"></i> Absent (0 pts)</span>';
                if(!empty($sess['joined_at'])) {
                    $startedTime = strtotime($sess['started_at']);
                    $joinedTime = strtotime($sess['joined_at']);
                    $diffMinutes = ($joinedTime - $startedTime) / 60;
                    if($diffMinutes >= 15) {
                        $statusHtml = '<span class="status-badge badge-pending" style="background:#fff3cd; color:#856404;"><i class="fa fa-clock-o"></i> Late (1 pt)</span>';
                    } else {
