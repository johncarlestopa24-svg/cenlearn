...
// Auto-record study plan snapshot into history if not already recorded today
$currentPlanItems = !empty($topic_recs['recommendations'])
  ? array_values(array_filter($topic_recs['recommendations'], fn($r) => $r['score_pct'] !== null || !empty($r['modules'])))
  : [];
if (!empty($currentPlanItems) || !empty($topic_recs['motivational_quote'])) {
    cenlearn_save_study_plan_snapshot(
        $conn,
        $uc,
        $student_analytics['overall_risk'] ?? 'on_track',
        $student_analytics['risk_score'] ?? 0,
        $student_analytics['recommendations'] ?? [],
        $currentPlanItems,
        $topic_recs['motivational_quote'] ?? null
    );
}
...
$safeCountStudent = function($sql) use ($conn) {
    try {
        $r = $conn->query($sql);
...
$classCount = $safeCountStudent("SELECT COUNT(*) AS c FROM class_members cm JOIN classes c ON cm.class_id=c.id WHERE cm.user_code='$uc' AND c.teacher_code!='$uc' AND (c.is_archived=0 OR c.is_archived IS NULL) AND (c.is_subject_only=0 OR c.is_subject_only IS NULL)");

// Quiz & Assignment KPI stats
$totalQuizzesCount = $safeCountStudent("SELECT COUNT(*) AS c FROM quizzes q JOIN class_members cm ON cm.class_id=q.class_id JOIN classes c ON q.class_id=c.id WHERE cm.user_code='$uc' AND q.is_active=1 AND (c.is_archived=0 OR c.is_archived IS NULL) AND (c.is_subject_only=0 OR c.is_subject_only IS NULL)");
$completedQuizzesCount = $safeCountStudent("SELECT COUNT(DISTINCT quiz_id) AS c FROM quiz_submissions WHERE student_code='$uc'");

$totalAssignCount = $safeCountStudent("SELECT COUNT(*) AS c FROM assignments a JOIN class_members cm ON cm.class_id=a.class_id JOIN classes c ON a.class_id=c.id WHERE cm.user_code='$uc' AND (c.is_archived=0 OR c.is_archived IS NULL) AND (c.is_subject_only=0 OR c.is_subject_only IS NULL)");
$completedAssignCount = $safeCountStudent("SELECT COUNT(DISTINCT assignment_id) AS c FROM assignment_submissions WHERE student_code='$uc'");
...
<aside class="sd-sidebar" id="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <ul>
      ...
      <li><a href="study_plan_history.php"><i class="fa fa-history"></i> Study Plan History</a></li>
    </ul>
  </nav>
  <div class="sb-footer">
    <div class="sb-user">
...
<div class="sd-main">
  ...
  <div class="sd-content">
    ...
    <div class="ai-assistant-card" style="margin-bottom:24px;border-top:4px solid #6366f1;">
      <div class="ai-hdr" style="margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <div class="ai-title">
          <i class="fa fa-book" style="color:#6366f1;"></i> Study Plan &amp; Learning Recommendations
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          <button type="button" class="btn-past-plans" onclick="$('#pastStudyPlansModal').modal('show')" style="border-radius:8px;font-weight:700;font-size:12px;padding:6px 14px;background:#ffffff;color:#4f46e5;border:1.5px solid #c7d2fe;box-shadow:0 1px 4px rgba(99,102,241,0.12);display:inline-flex;align-items:center;gap:6px;cursor:pointer;transition:all 0.2s;">
            <i class="fa fa-history" style="color:#6366f1;font-size:13px;"></i> Past Study Plans &amp; Recommendations
            <span style="background:#6366f1;color:#fff;font-size:10px;font-weight:800;padding:2px 7px;border-radius:10px;"><?php echo $historyCount; ?></span>
          </button>
...
<!-- ── Past Study Plans & Learning Recommendations Modal ── -->
<div class="modal fade" id="pastStudyPlansModal" tabindex="-1" role="dialog" aria-labelledby="pastStudyPlansModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document" style="max-width:880px;">
    <div class="modal-content" style="border-radius:16px;border:none;overflow:hidden;box-shadow:0 20px 40px rgba(0,0,0,0.18);">
      
      <!-- Modal Header -->
      <div class="modal-header" style="background:linear-gradient(135deg,#4f46e5 0%,#3b82f6 100%);color:#fff;padding:20px 24px;border:none;">
        <div style="display:flex;align-items:center;justify-content:space-between;width:100%;">
          <div style="display:flex;align-items:center;gap:14px;">
            <div style="width:42px;height:42px;border-radius:12px;background:rgba(255,255,255,0.2);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;font-size:18px;color:#fff;">
              <i class="fa fa-history"></i>
            </div>
            <div>
              <h4 class="modal-title" id="pastStudyPlansModalLabel" style="font-size:17px;font-weight:800;color:#fff;margin:0;">
                Past Study Plans &amp; Learning Recommendations
              </h4>
              <p style="font-size:12px;color:rgba(255,255,255,0.85);margin:2px 0 0;">
                Review your historical diagnostic feedback, topic improvement targets, and learning milestones over time.
              </p>
            </div>
          </div>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="color:#fff;opacity:0.85;font-size:24px;text-shadow:none;outline:none;">
      ...
      <!-- Modal Body -->
      <div class="modal-body" style="background:#f8fafc;padding:22px 24px;max-height:calc(85vh - 120px);overflow-y:auto;">
        
        <?php if(empty($studyPlanHistory)): ?>
        <div style="text-align:center;padding:48px 20px;background:#fff;border-radius:14px;border:1px solid #e2e8f0;">
          <div style="width:56px;height:56px;border-radius:50%;background:#ede9fe;color:#6366f1;display:inline-flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:12px;">
            <i class="fa fa-calendar-check-o"></i>
          </div>
          <h5 style="font-size:16px;font-weight:700;color:#0f172a;margin-bottom:6px;">No Past Study Plans Recorded Yet</h5>
          <p style="font-size:13px;color:#64748b;max-width:440px;margin:0 auto;">
            As you complete quizzes, assignments, and learning activities, CenLearn automatically archives your personalized study plans so you can monitor your academic growth.
          </p>
        </div>
        <?php else: ?>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:12px;margin-bottom:20px;">
          <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:12px 16px;display:flex;align-items:center;gap:12px;">
            <div style="width:38px;height:38px;border-radius:10px;background:#e0e7ff;color:#4f46e5;display:flex;align-items:center;justify-content:center;font-size:16px;">
              <i class="fa fa-archive"></i>
            </div>
            <div>
              <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;">Archived Plans</div>
              <strong style="font-size:18px;color:#0f172a;font-weight:800;"><?php echo count($studyPlanHistory); ?> Snapshot<?php echo count($studyPlanHistory)!==1?'s':''; ?></strong>
            </div>
          </div>

          <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:12px 16px;display:flex;align-items:center;gap:12px;">
            <div style="width:38px;height:38px;border-radius:10px;background:#dcfce7;color:#10b981;display:flex;align-items:center;justify-content:center;font-size:16px;">
              <i class="fa fa-clock-o"></i>
            </div>
            <div>
              <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;">First Recorded</div>
              <strong style="font-size:13px;color:#0f172a;font-weight:700;">
                <?php echo date('M d, Y', strtotime(end($studyPlanHistory)['created_at'])); ?>
              </strong>
            </div>
          </div>

          <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:12px 16px;display:flex;align-items:center;gap:12px;">
            <div style="width:38px;height:38px;border-radius:10px;background:#fef3c7;color:#d97706;display:flex;align-items:center;justify-content:center;font-size:16px;">
              <i class="fa fa-refresh"></i>
            </div>
            <div>
              <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;">Latest Update</div>
              <strong style="font-size:13px;color:#0f172a;font-weight:700;">
                <?php echo date('M d, Y', strtotime($studyPlanHistory[0]['created_at'])); ?>
              </strong>
            </div>
          </div>
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
          <?php foreach($studyPlanHistory as $hIdx => $hRow):
            $hDate = date('F j, Y \a\t g:i A', strtotime($hRow['created_at']));
            $hRisk = $hRow['overall_risk'] ?: 'on_track';
            $hRiskScore = intval($hRow['risk_score']);
            $riskBadge = [
            ...
            ][$hRisk] ?? ['bg'=>'#f1f5f9','color'=>'#475569','border'=>'#e2e8f0','label'=>ucwords($hRisk)];

            $hQuote = $hRow['quote'] ?? null;
            $hItems = $hRow['topic_items'] ?? [];
            $collapseId = 'hCollapse_'.$hRow['id'];
          ?>
          <div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;box-shadow:0 2px 6px rgba(0,0,0,0.03);overflow:hidden;">
            
            <!-- Snapshot Top Bar -->
            <div style="padding:14px 18px;background:#ffffff;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;cursor:pointer;" onclick="$('#<?php echo $collapseId; ?>').slideToggle(200); $(this).find('.toggle-chevron').toggleClass('fa-chevron-down fa-chevron-up');">
              <div style="display:flex;align-items:center;gap:10px;">
                <span style="width:28px;height:28px;border-radius:8px;background:#e0e7ff;color:#4f46e5;display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;">
                  #<?php echo count($studyPlanHistory) - $hIdx; ?>
                </span>
                <div>
                  <div style="font-size:13.5px;font-weight:700;color:#0f172a;display:flex;align-items:center;gap:6px;">
                    <i class="fa fa-calendar" style="color:#6366f1;font-size:12px;"></i> <?php echo $hDate; ?>
                    <?php if($hIdx === 0): ?>
                    <span style="font-size:10px;font-weight:800;background:#dcfce7;color:#166534;padding:1px 6px;border-radius:6px;text-transform:uppercase;">Current</span>
                    <?php endif; ?>
                  </div>
                  <div style="font-size:11.5px;color:#64748b;margin-top:2px;">
                    <?php echo count($hItems); ?> topic recommendation<?php echo count($hItems)!==1?'s':''; ?> recorded
                  </div>
                </div>
              </div>

              <div style="display:flex;align-items:center;gap:8px;">
                <span style="font-size:11.5px;font-weight:700;background:<?php echo $riskBadge['bg']; ?>;color:<?php echo $riskBadge['color']; ?>;border:1px solid <?php echo $riskBadge['border']; ?>;padding:3px 10px;border-radius:99px;">
                  <i class="fa fa-shield"></i> <?php echo $riskBadge['label']; ?> (Risk: <?php echo $hRiskScore; ?>/100)
                </span>
                <i class="fa fa-chevron-<?php echo $hIdx === 0 ? 'up' : 'down'; ?> toggle-chevron" style="color:#94a3b8;font-size:12px;margin-left:4px;"></i>
              </div>
            </div>

            <!-- Expandable Snapshot Body -->
            <div id="<?php echo $collapseId; ?>" style="padding:16px 18px;<?php echo $hIdx > 0 ? 'display:none;' : ''; ?>">
              
              <!-- Historical AI Coach Quote if available -->
              <?php if(!empty($hQuote)): ?>
              <div style="background:<?php echo $hQuote['bg'] ?? '#fffbeb'; ?>;border:1px solid <?php echo $hQuote['border'] ?? '#fde68a'; ?>;border-left:4px solid <?php echo $hQuote['color'] ?? '#f59e0b'; ?>;border-radius:10px;padding:12px 14px;margin-bottom:14px;display:flex;align-items:flex-start;gap:10px;">
                <i class="fa <?php echo $hQuote['icon'] ?? 'fa-graduation-cap'; ?>" style="color:<?php echo $hQuote['color'] ?? '#f59e0b'; ?>;font-size:15px;margin-top:2px;"></i>
                <div style="flex:1;">
                  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:4px;">
                    <strong style="font-size:12.5px;color:#0f172a;"><?php echo $hQuote['title'] ?? 'AI Diagnostic Coach'; ?></strong>
                    <span style="font-size:10.5px;font-weight:800;color:<?php echo $hQuote['color'] ?? '#f59e0b'; ?>;"><?php echo htmlspecialchars($hQuote['badge'] ?? ''); ?></span>
                  </div>
                  <div style="font-size:12px;color:#334155;line-height:1.45;">
                    <?php echo $hQuote['message'] ?? ''; ?>
                  </div>
                </div>
              </div>
              <?php endif; ?>

              <!-- Historical Recommendations Items -->
              <?php if(!empty($hItems)): ?>
              <div style="display:flex;flex-direction:column;gap:12px;">
                <?php foreach($hItems as $itemIdx => $item):
                ...
                ?>
                <div style="background:#f8fafc;border:1px solid #e2e8f0;border-left:3px solid #6366f1;border-radius:10px;padding:12px 14px;">
                  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:6px;">
                    <strong style="font-size:13px;color:#0f172a;">
                      <?php echo $iTitle; ?>
                    </strong>
                    <?php if($iScore !== null): ?>
                    <span style="font-size:11px;font-weight:800;background:<?php echo $iScore < 50 ? '#fee2e2' : ($iScore < 75 ? '#fef3c7' : '#dcfce7'); ?>;color:<?php echo $iScore < 50 ? '#991b1b' : ($iScore < 75 ? '#92400e' : '#166534'); ?>;padding:2px 8px;border-radius:99px;">
                      Past Score: <?php echo $iScore; ?>% <?php echo $iScore < 75 ? '(Developing)' : '(Mastered)'; ?>
                    </span>
                    <?php endif; ?>
                  </div>

                  <p style="font-size:12px;color:#475569;margin-bottom:8px;line-height:1.45;">
                    <?php echo $iDesc; ?>
                  </p>

                  <!-- Phased Plan Summary from this snapshot -->
                  <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:8px;margin-top:8px;">
                    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:7px;padding:8px 10px;font-size:11px;">
                      <strong style="color:#4f46e5;display:block;margin-bottom:3px;font-size:10px;text-transform:uppercase;">
                        <i class="fa fa-book"></i> Phase 1: Module Study
                      </strong>
                      <span style="color:#64748b;">
                        <?php echo !empty($item['primary_module']) ? htmlspecialchars($item['primary_module']['title']) : 'Review tagged class module'; ?>
                      </span>
                    </div>

                    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:7px;padding:8px 10px;font-size:11px;">
                      <strong style="color:#d97706;display:block;margin-bottom:3px;font-size:10px;text-transform:uppercase;">
                        <i class="fa fa-pencil"></i> Phase 2: Practice Retake
                      </strong>
                      <span style="color:#64748b;">
                        Analyze missed items &amp; retake assessment
                      </span>
                    </div>

                    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:7px;padding:8px 10px;font-size:11px;">
                      <strong style="color:#10b981;display:block;margin-bottom:3px;font-size:10px;text-transform:uppercase;">
                        <i class="fa fa-bullseye"></i> Phase 3: Benchmark
                      </strong>
                      <span style="color:#64748b;">
                        Target &ge; 75% standard proficiency
                      </span>
                    </div>
                  </div>

                  <?php if(!empty($iStd) && !empty($iStd['bloom_level'])): ?>
                  <div style="margin-top:8px;font-size:10.5px;color:#6366f1;background:#ede9fe;padding:4px 8px;border-radius:6px;display:inline-flex;align-items:center;gap:5px;">
                    <i class="fa fa-bookmark"></i> Standard: <?php echo htmlspecialchars($iStd['bloom_level']); ?> &bull; <?php echo htmlspecialchars($iStd['standard_code'] ?? ''); ?>
                  </div>
                  <?php endif; ?>
      ...
      <!-- Modal Footer -->
      <div class="modal-footer" style="background:#fff;padding:12px 24px;border-top:1px solid #e2e8f0;display:flex;align-items:center;justify-content:space-between;">
        <a href="study_plan_history.php" class="btn btn-sm" style="background:#f1f5f9;color:#334155;border:1px solid #cbd5e1;font-weight:600;font-size:12px;border-radius:8px;display:inline-flex;align-items:center;gap:6px;">
          <i class="fa fa-external-link"></i> Open Full History Report
        </a>
        <button type="button" class="btn btn-sm" data-dismiss="modal" style="background:#4f46e5;color:#fff;font-weight:700;font-size:12px;padding:7px 18px;border-radius:8px;border:none;">
          Close
        </button>
      </div>
