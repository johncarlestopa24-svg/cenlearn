...
/**
 * Automatically record a study plan and learning recommendation snapshot into
 * `student_study_plan_history` if not already saved today, or if forced.
 */
function cenlearn_save_study_plan_snapshot($conn, $student_code, $overall_risk, $risk_score, $recommendations, $topic_plans, $quote = null, $force = false): bool {
    $uc = $conn->real_escape_string($student_code);
    $risk = $conn->real_escape_string($overall_risk ?: 'on_track');
    $score = intval($risk_score);

    if (!$force) {
        $chk = $conn->query("SELECT id, created_at, risk_score, topic_plans_json FROM student_study_plan_history WHERE student_code = '$uc' ORDER BY created_at DESC LIMIT 1");
        if ($chk && $chk->num_rows > 0) {
            $last = $chk->fetch_assoc();
            $lastDate = date('Y-m-d', strtotime($last['created_at']));
            $today = date('Y-m-d');
            
            // If already recorded today, check if topic performance or score has changed
            if ($lastDate === $today) {
                $lastPayload = json_decode($last['topic_plans_json'] ?: '[]', true);
                $lastItems = isset($lastPayload['items']) ? $lastPayload['items'] : (is_array($lastPayload) ? $lastPayload : []);
                
                $lastSig = array_map(fn($it) => ($it['topic']??'').'_'.($it['score_pct']??''), $lastItems);
                $currSig = array_map(fn($it) => ($it['topic']??'').'_'.($it['score_pct']??''), $topic_plans ?: []);
                
                if ($lastSig === $currSig && intval($last['risk_score']) === $score) {
                    return false; // Identical, no change needed
                }
                
                // Update today's snapshot with the newly tracked lowest topics and scores
                $lastId = intval($last['id']);
                $recsJson = $conn->real_escape_string(json_encode($recommendations ?: [], JSON_UNESCAPED_UNICODE));
                $payload = ['items' => $topic_plans ?: [], 'quote' => $quote];
                $plansJson = $conn->real_escape_string(json_encode($payload, JSON_UNESCAPED_UNICODE));
                return (bool)$conn->query("UPDATE student_study_plan_history SET overall_risk='$risk', risk_score=$score, recommendations_json='$recsJson', topic_plans_json='$plansJson', created_at=NOW() WHERE id=$lastId");
            }
        }
    }
    ...
    return (bool)$conn->query("INSERT INTO student_study_plan_history (student_code, overall_risk, risk_score, recommendations_json, topic_plans_json, created_at) VALUES ('$uc', '$risk', $score, '$recsJson', '$plansJson', NOW())");
}

/**
 * Fetch all past study plans and recommendations history for a student.
 */
function cenlearn_get_study_plan_history($conn, $student_code): array {
    $uc = $conn->real_escape_string($student_code);
    $res = $conn->query("
        SELECT id, student_code, overall_risk, risk_score, recommendations_json, topic_plans_json, created_at
        FROM student_study_plan_history
        WHERE student_code = '$uc'
        ORDER BY created_at DESC
    ");
    $list = [];
    if ($res) {
        while ($r = $res->fetch_assoc()) {
            $tJson = json_decode($r['topic_plans_json'] ?: '[]', true);
            $items = isset($tJson['items']) ? $tJson['items'] : (is_array($tJson) ? $tJson : []);
            $quote = $tJson['quote'] ?? null;
            $r['topic_items'] = $items;
            $r['quote']       = $quote;
            $r['recs']        = json_decode($r['recommendations_json'] ?: '[]', true) ?: [];
            $list[] = $r;
        }
    }
    return $list;
