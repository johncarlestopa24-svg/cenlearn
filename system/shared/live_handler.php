...
if($action === 'record_attendance' && $role === 'STUDENT'){
    ...
    if($colQ->num_rows === 0){
        ...
        $conn->query("INSERT INTO class_record_columns
                      ...
                      VALUES ($class_id, 'attendance', '$attTitle', 2.00, 0, $session_id, '$termToUse')");
        $col_id = $conn->insert_id;
    } else {
        $col_id = intval($colQ->fetch_assoc()['id']);
        // Ensure max_score is updated to 2.00 and component is attendance
        $conn->query("UPDATE class_record_columns SET component = 'attendance', max_score = 2.00 WHERE id = $col_id");
    }
...
if($action === 'attendance' && $role === 'TEACHER'){
    ...
    $sq = $conn->query("SELECT started_at, scheduled_at FROM live_sessions WHERE id=$session_id LIMIT 1");
    $sessInfo = ($sq && $sq->num_rows > 0) ? $sq->fetch_assoc() : null;
    $sessStart = $sessInfo['started_at'] ?? $sessInfo['scheduled_at'] ?? null;
    $startedTs = $sessStart ? strtotime($sessStart) : 0;
    ...
    while($r = $res->fetch_assoc()){
        $joinedTs = strtotime($r['joined_at']);
        $diffMins = ($startedTs > 0 && $joinedTs > $startedTs) ? ($joinedTs - $startedTs)/60 : 0;
        $score = ($diffMins >= 15) ? 1.00 : 2.00;
        $r['score'] = $score;
        $r['status_label'] = ($diffMins >= 15) ? 'Late (1 pt)' : 'Present (2 pts)';
        $list[] = $r;
    }
